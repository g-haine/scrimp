...
class dpHs:
    ...
    def add_state(self, name, description, kind, region=None, mesh_id=0):
        ...
        print('A state variable', name, ', describing \'', description, '\', has been initialized as a', kind, 'on mesh', mesh_id, where)
    ...
    def add_brick(self, name, form, regions, linear=True, dt=False, position='constitutive', mesh_id=0):
        ...
        if linear:
            for region in regions:
                ...
                print('Linear form \'', self.bricks[name]['form'], '\' has been added as', position, 'relation on region', region, 'of mesh', mesh_id)
        else:
            for region in regions:
                id_brick = self.gf_model.add_nonlinear_term(self.domain.int_method[mesh_id], 
                                                            form, region)
                self.bricks[name]['id_bricks'].append(id_brick)
                print('Non-linear form \'', self.bricks[name]['form'], '\' has been added as', position, 'relation on region', region, 'of mesh', mesh_id)
    
    def add_control_port(self, name, name_control, description_control, name_observation, description_observation, kind, region=None, position='effort', mesh_id=0):
        """
    ...
    def set_control(self, name, expression):
        ...
        source_id = self.gf_model.add_source_term(self.domain.int_method[self.controls[name]['mesh_id']], expression_form, self.controls[name]['region'])
    ...
    def solve(self):
        ...
        for name_variable in self.initial_value_setted.keys():
            assert self.initial_value_setted[name_variable], ('Initial value of', name_variable,'must be defined before time-resolution')
...
class domain:
    ...
    def __init__(self):
        ...
        self.subdomains = list(dict()) #: A list of dict `str`: `int` listing the indices of region of dim n in the getfem mesh
        self.boundaries = list(dict()) #: A list of dict `str`: `int` listing the indices of region of dim n-1 in the getfem mesh
