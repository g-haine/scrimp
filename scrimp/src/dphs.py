class dpHs:
    ...
    def __init__(self, basis_field='real'):
        ...
        self.domain = domain()  #: The `domain` of a dpHs is an object that handle mesh(es) and dict of regions with getfem indices (for each mesh), useful to define `bricks` (i.e. forms) in the getfem syntax
        ...
        self.costates = dict()  #: The dict of `costates`, store many infos for display()
        self.ports = dict()  #: The dict of `ports`, store many infos for display()
        self.bricks = dict()  #: The dict of `bricks`, associating getfem `bricks` to petsc matrices obtained by the PFEM, store many infos for display()
        self.controls = dict()  #: The dict of `controls`, collecting information about control ports. Also appear in `ports` entries
        self.Hamiltonian = list()  #: The `Hamiltonian` of a dpHs is a list of dict containing several useful information for each term
        ...
        self.tangent_mass = PETSc.Mat().create(
            comm=comm)  #: Tangent (non-linear + linear) mass matrix of the system in PETSc CSR format
        ...
        self.tangent_stiffness = PETSc.Mat().create(
            comm=comm)  #: Tangent (non-linear + linear) stiffness matrix of the system in PETSc CSR format
        self.nl_stiffness = PETSc.Mat().create(
            comm=comm)  #: Non-linear stiffness matrix of the system in PETSc CSR format
        ...
        self.time_scheme[
            'isset'] = False  #: To check if the PETSc TS time-integration parameters have been setted before time-integration
    ...
    def add_state(self, name, description, kind, region=None, mesh_id=0):
        ...
        print('A state variable', name, ', describing \'', description, '\', has been initialized as a', kind,
              'on mesh', mesh_id, where)
    ...
    def add_costate(self, name, description, state, substituted=False):
        ...
        assert state in self.states.keys(), ('State', state, 'must be added before its co-state')
    ...
    def add_port(self, name, flow, effort, kind, mesh_id=0, algebraic=True, substituted=False, region=None):
        ...
        print('A port \'', name, '\' (', flow, ',', effort, ') of', kind, 'type has been added on mesh', mesh_id, where)
    ...
    def add_FEM(self, name_port, order, FEM='CG'):
        ...
        self.ports[name_port].set_FEM(self.domain.mesh[self.ports[name_port].mesh_id],
                                      dim, order, FEM)
        ...
        if self.ports[name_port].region is not None:
            self.gf_model.add_filtered_fem_variable(self.ports[name_port].flow,
        ...
        if self.ports[name_port].effort in self.costates.keys():
            ...
            'Port', name_port, 'should not be algebraic because', self.ports[name_port].effort, 'is a co-state')
    ...
    def add_brick(self, name, form, regions, linear=True, dt=False, position='constitutive', mesh_id=0):
        """
        ...
        if linear:
            for region in regions:
                id_brick = self.gf_model.add_linear_term(self.domain.int_method[mesh_id],
                                                         form, region)
                self.bricks[name]['id_bricks'].append(id_brick)
                print('Linear form \'', self.bricks[name]['form'], '\' has been added as', position,
                      'relation on region', region, 'of mesh', mesh_id)
        else:
            for region in regions:
                id_brick = self.gf_model.add_nonlinear_term(self.domain.int_method[mesh_id],
                                                            form, region)
                self.bricks[name]['id_bricks'].append(id_brick)
                print('Non-linear form \'', self.bricks[name]['form'], '\' has been added as', position,
                      'relation on region', region, 'of mesh', mesh_id)

    def add_control_port(self, name, name_control, description_control, name_observation, description_observation, kind,
                         region=None, position='effort', mesh_id=0):
        """
    ...
    def set_control(self, name, expression):
        ...
        self.add_brick('M_' + u, mass_form, [self.controls[name]['region']], linear=True, dt=False,
                       position='constitutive', mesh_id=self.controls[name]['mesh_id'])

        # Construct the form
        expression_form = '-(' + expression + ')' + times + 'Test_' + u
        # Add the source brick
        source_id = self.gf_model.add_source_term(self.domain.int_method[self.controls[name]['mesh_id']],
                                                  expression_form, self.controls[name]['region'])

        self.bricks[u + '_source'] = {'id_bricks': [source_id],
        ...
        print('Control function has been setted to', u, '=', expression, 'on region', self.controls[name]['region'],
              'of mesh', self.controls[name]['mesh_id'])
    ...
    def assemble_mass(self):
        ...
        self.mass = extract_gmm_to_petsc([0, size], [0, size],
    ...
    def assemble_stiffness(self):
        ...
        self.gf_model.assembly(option='build_matrix')
        size = self.gf_model.nbdof()
        self.stiffness = extract_gmm_to_petsc([0, size], [0, size],
                                              self.gf_model.tangent_matrix())
    ...
    def assemble_nl_mass(self):
        ...
        self.gf_model.assembly(option='build_matrix')
        size = self.gf_model.nbdof()
        self.nl_mass = extract_gmm_to_petsc([0, size], [0, size],
                                            self.gf_model.tangent_matrix())
    ...
    def assemble_nl_stiffness(self):
        ...
        self.gf_model.assembly(option='build_matrix')
        size = self.gf_model.nbdof()
        self.nl_stiffness = extract_gmm_to_petsc([0, size], [0, size],
                                                 self.gf_model.tangent_matrix())
    ...
    def solve(self):
        ...
        assert self.time_scheme['isset'], ('Time scheme must be defined before time-resolution')
        for name_variable in self.initial_value_setted.keys():
            assert self.initial_value_setted[name_variable], (
            'Initial value of', name_variable, 'must be defined before time-resolution')
        for name_control in self.controls.keys():
            assert self.controls[name_control]['isset'], (
            'Control', name_control, 'must be defined before time-resolution')
    ...
    def monitor(self, TS, i, t, z, dt_save=1, t_0=0., initial_step=False):
        """
        ...
        if not initial_step:
            ...
            if (next_saved_at_t - t <= 0.1 * TS.getTimeStep()) or (i == -1):
                sys.stdout.write(f"\ri={i:8d} t={t:8g} * ({int(time.time() - self.ts_start)}s)         \n")
                sys.stdout.flush()
    ...
    def compute_Hamiltonian(self):
        ...
        assert self.solve_done, ('System has not been solved yet, Hamiltonian can not be computed')
        ...
        for t in range(len(self.solution['t'])):
            ...
            for term in range(len(self.Hamiltonian)):
                ...
                for region in self.Hamiltonian[term]['regions']:
                    term_value_at_t += gf.asm('generic', self.domain.int_method[self.Hamiltonian[term]['mesh_id']], 0,
                                              self.Hamiltonian[term]['expression'], region, self.gf_model)
        ...
        for name_port in self.ports.keys():
            if self.ports[name_port].algebraic and not self.ports[name_port].substituted:
                ...
                if self.ports[name_port].kind == 'scalar-field':
                    ...
                    raise ValueError('Unknown kind', self.ports[name_port].kind, 'for control port')
                ...
                form = minus + self.ports[name_port].flow + times + self.ports[name_port].effort

                self.powers[name_port] = []
                for t in range(len(self.solution['t'])):
                    self.gf_model.to_variables(self.solution['z'][t])

                    power_value_at_t = gf.asm('generic', self.domain.int_method[self.ports[name_port].mesh_id], 0,
                                              form, region, self.gf_model)
        ...
        for name in self.bricks.keys():
            ...
            if brick['position'] == 'flow':
                for region in brick['regions']:
                    matrix = gf.asm_generic(self.domain.int_method[brick['mesh_id']],
                                            2, brick['form'], region, self.gf_model)
                    M += convert_PETSc_to_scipy(extract_gmm_to_petsc([0, size], [0, size], matrix))
            ...
            with open(os.path.join(path, name_variable + '.pvd'), 'w') as pvd_file:
                ...
                for k in range(len(self.solution['t'])):
                    ...
                    FEM.export_to_vtu(os.path.join(path, name_variable + f'_{k:05d}.vtu'), 'ascii',
                                      FEM, z, name_variable)
                    pvd_file.write('    <DataSet timestep="' + str(
                        self.solution['t'][k]) + '" file="' + name_variable + f'_{k:05d}.vtu' + '" />\n')
