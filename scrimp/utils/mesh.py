...
def built_in_geometries():
    ...
    print('')
    print('* `Concentric`, {"R": 1, "r": 0.6, "h": 0.1}, a disk of radius r surrounded by an annulus of radii r and R with mesh size h')
    print('---> Domain `Omega_Disk`: 1, `Omega_Annulus`: 2, Interface `Interface`: 10, Boundary `Gamma`: 20')
    print('')
...
def Concentric(parameters={'R': 1., 'r': 0.6, 'h': 0.1}, terminal=1):
    """
    ...
    points_ext = []
    for j in range(3):
        points_int.append(model.geo.addPoint(r*math.cos(2*math.pi*j/3), r*math.sin(2*math.pi*j/3), 0, h, j+101))
        points_ext.append(model.geo.addPoint(R*math.cos(2*math.pi*j/3), R*math.sin(2*math.pi*j/3), 0, h, j+201))
    
    lines_int = []
    lines_ext = []
    for j in range(3):
        lines_int.append(model.geo.addCircleArc(points_int[j],center,points_int[(j+1)%3], j+10))
        lines_ext.append(model.geo.addCircleArc(points_ext[j],center,points_ext[(j+1)%3], j+20))
    # Curveloop and Surface
    ...
    print('A disk of radius '+str(r)+' surrounded by an annulus of radii '+str(r)+' and '+str(R)+' has been meshed')
    
    mesh = gf.Mesh('import', 'gmsh_with_lower_dim_elt', os.path.join(path, 'Concentric.msh'))
    mesh.region_merge(10, 11)
    mesh.region_merge(10, 12)
    mesh.delete_region([11,12])
